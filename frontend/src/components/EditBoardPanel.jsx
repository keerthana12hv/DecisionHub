...
export default function EditBoardPanel({ decision, onSaved, onCancel }) {
  ...
  // ACTIVE / CLOSED = only Poll End Time can be changed.
  ...
  const [votingEndTime, setVotingEndTime] = useState(
    decision.votingEndTime
      ? parseLocalDateTimeForInput(decision.votingEndTime)
      : ""
  );
  ...
  // Save all editable draft fields.
  const saveDraftEdits = async () => {
    // 1. Update decision details
    const payload = {
      title,
      description: decision.categoryName
        ? `[Cat:${decision.categoryName}] ${description}`
        : description,
    ...
    // 2. Update changed options
    const changedOptions = options.filter((opt) => {
      const original = (decision.options || []).find(
        (o) => o.id === opt.id
      );
    ...
    // 3. Delete and create rating criteria
    if (decision.votingType === "RATING_BASED") {
      const deletedFactors = (decision.factors || []).filter(
        (f) => !criteria.includes(f.name)
      );
      ...
      for (const factor of deletedFactors) {
        await api.delete(
          `/decisions/${decision.id}/factors/${factor.id}`
        );
      }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Fetch poll information after publishing
      if (
        updatedDecision &&
        updatedDecision.status !== "DRAFT"
      ) {
        try {
          const pollRes = await api.get(
            `/decisions/${decision.id}/poll`
          );
          ...
          if (pollRes.data) {
            ...
            if (pollRes.data.endTime) {
              updatedDecision.votingEndTime =
                pollRes.data.endTime;
            }
          }
        } catch (pollErr) {
          console.error(
            "Failed to fetch poll details post-publish:",
            pollErr
          );
        }
      }

      // Return updated decision to DecisionDetail
      ...
      let errMsg =
        "Could not publish this decision. Check console for details.";
  ...
  const handleSave = async () => {
    ...
    try {
      if (isDraft) {
        // Draft: save all editable fields
        await saveDraftEdits();
      } else {
        // Published/Closed:
        // ONLY Poll End Time can be changed
        await api.patch(
      ...
      // Re-fetch decision after saving
      ...
      // Fetch poll information for ACTIVE/CLOSED decisions
      if (
        updatedDecision &&
        updatedDecision.status !== "DRAFT"
      ) {
        try {
          ...
          console.error(
            "Failed to fetch poll details post-save:",
            pollErr
          );
        }
      }
    ...
    <div className="edit-board-panel">
      ...
      {isDraft ? (
        <p className="section-subtitle">
          This decision is still a Draft — everything below can
          be edited before you publish.
        </p>
      ) : (
        <p className="section-subtitle">
          This decision has been published. Only the Poll End
          Time can still be changed.
        </p>
      )}
      ...
      <div className="form-group">
        ...
        <p className="section-subtitle">
          When voting closes. This is always editable, even
          after publishing.
        </p>
      </div>
      ...
      {decision.votingType === "RATING_BASED" && (
        <>
          ...
          <p className="section-subtitle">
            Names of what voters will rate each option on.
            Scores are entered by voters during voting, not
            here.
          </p>
